...
export function PilarCard({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`pointer-events-none absolute bottom-[-10px] right-3.5 select-none font-serif text-[72px] leading-none text-gray-1 transition-colors duration-200 ${bgNumHover}`}
      >
        {bgNum}
      </div>
      {icon && (
      ...
      <div className={`relative mb-1 text-[10px] font-semibold uppercase tracking-wider ${numColor}`}>
        {numLabel}
      </div>
      {tag && (
        <span
          className={`relative mb-2 inline-block rounded-full px-2.5 py-0.5 text-[10px] font-semibold ${
      ...
      <h3 className="relative mb-2 text-base font-semibold leading-snug text-dark">{title}</h3>
      {statValue && (
        <div className="relative mb-2.5 inline-flex items-baseline gap-1">
          <span className={`font-serif text-[30px] tracking-tight ${numColor}`}>{statValue}</span>
          <span className="max-w-[5rem] text-[11px] leading-tight text-gray-3">{statLabel}</span>
        </div>
      )}
      <p className="relative text-[13px] leading-relaxed text-gray-3">{children}</p>
    </div>
  );
}
